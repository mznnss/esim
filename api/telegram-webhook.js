...
// Katalog Paket eSIM untuk Pembeli di Telegram
const PACKAGES = {
  "PKG_SG_5GB": { name: "ASIA (INDO-SGP-MLY) - 5 GB High Speed (30 Hari)", price: 40000 },
  "PKG_ASIA_10GB": { name: "ASIA (INDO-SGP-MLY) - 10 GB High Speed (15 Hari)", price: 40000 },
  "PKG_JP_10GB": { name: "JAPAN UNLIMITED - 10 GB High Speed (7 Hari)", price: 120000 }
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const isAdmin = chatIdStr === GROUP_ADMIN_ID || Object.keys(ADMIN_NAMES).includes(senderIdStr);

    // ========================================================
    // 1. AREA ADMIN (GRUP ATAU JAPRI ADMIN)
    // ========================================================

    // 1.1 TANGANI COMMAND TEKS ADMIN (/start, /pantau, /cek, /selesai)
    if (isAdmin && update.message && update.message.text) {
...
*(Opsional: Tulis catatan pada caption foto/dokumen).*`,
        undefined,
        update.callback_query.message.message_id
      );

      // Info ke pembeli jika pesan lewat bot
    ...
    if (isAdmin && update.message && (update.message.document || update.message.photo) && update.message.reply_to_message) {
      ...
      if (match) {
        ...
        // A. Kirim ke Chat Pribadi Pembeli (Jika ada)
        if (orderData.buyer_chat_id) {
...
4. Aktifkan *Data Roaming* saat telah tiba di negara tujuan.`;
        ...
        // B. Kirim Email Dark Mode Persis Web
        if (orderData.email) {
...
_QR Code / PDF telah sukses terkirim ke email & Telegram pembeli._`;
    ...
    // ========================================================
    // 2. AREA PEMBELI (CHAT PRIBADI PEMBELI / JAPRI)
    // ========================================================
...
Silakan klik tombol di bawah untuk memilih paket eSIM:`, welcomeKeyboard);
      ...
      // 2.2 PEMBELI UPLOAD FOTO BUKTI TRANSFER
      if (update.message.photo) {
        ...
        // Kirim bukti bayar ke grup admin
        await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendPhoto`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
...
📧 *Email:* ${orderInfo.email}`,
      ...
      if (text.includes('@') && text.includes('.')) {
        ...
        if (draftOrder) {
          ...
          await sendTelegramMsg(buyerChatId, `📋 *INVOICE PEMBAYARAN*
━━━━━━━━━━━━━━━━━━
🆔 *Order ID:* \`${orderId}\`
📦 *Paket:* ${orderInfo.package_name}
